...
function StatusBadge({ status }: { status: WorkstationStatus }) {
  return (
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium capitalize ${STATUS_STYLES[status]}`}
    >
      {status}
    </span>
  );
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 space-y-1.5 text-sm">
        ...
        {card.current && (
          <div className="text-xs text-muted-foreground">Operation: {ACTIVITY_OP_NAME[card.current.operationId]}</div>
        )}
      </div>
