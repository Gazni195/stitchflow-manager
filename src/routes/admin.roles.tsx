...
import { useSyncPermissions } from "@/lib/rbac/use-permission-sync";
          ...
          <>
                  ...
                  <button
                    onClick={() =>
                      sync.mutate(undefined, {
                        onSuccess: (r: { inserted: string[]; updated: string[]; unchanged: number }) =>
                          toast.success(
                            `Permissions synced — ${r.inserted.length} added, ${r.updated.length} updated, ${r.unchanged} unchanged.`,
                          ),
                        onError: (e: any) => toast.error(e?.message ?? "Sync failed"),
                      })
                    }
                    disabled={sync.isPending}
                    className="flex items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-2 text-xs font-bold hover:bg-accent disabled:opacity-60"
                  >
                    {sync.isPending ? (
            ...
            <div className="mb-3 flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-card p-3">
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                <button
                  onClick={expandAll}
                  className="rounded-lg border border-border bg-background px-2.5 py-1.5 text-[11px] font-bold hover:bg-accent"
                >
                  Expand All
                </button>
                <button
      ...
      <div className="grid gap-4 p-5">
        ...
        {create.isError && (
          <p className="text-xs text-destructive">
            {create.error instanceof Error ? create.error.message : "Failed to create role."}
          </p>
        )}
      </div>
  ...
  return (
    <DialogShell title="Edit Role" subtitle={role.key} onClose={onClose}>
      <div className="grid gap-4 p-5">
        ...
        {update.isError && (
          <p className="text-xs text-destructive">
            {update.error instanceof Error ? update.error.message : "Failed to update role."}
          </p>
        )}
      </div>
      <DialogFooter onCancel={onClose}>
  ...
  return (
    <DialogShell title="Clone Role" subtitle={`Copies every permission from “${role.label}”`} onClose={onClose}>
      <div className="grid gap-4 p-5">
        ...
        {clone.isError && (
          <p className="text-xs text-destructive">
            {clone.error instanceof Error ? clone.error.message : "Failed to clone role."}
          </p>
        )}
      </div>
      <DialogFooter onCancel={onClose}>
      ...
      <div className="px-5 py-4 text-sm text-muted-foreground">
        ...
        {role.userCount > 0 && (
          <p className="mt-2 font-semibold text-destructive">
            {role.userCount} user{role.userCount === 1 ? " is" : "s are"} still assigned to this role &mdash; deletion
            will be blocked until they're reassigned.
          </p>
        )}
      </div>
      <DialogFooter onCancel={onClose}>
