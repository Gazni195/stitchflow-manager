...
export type ProductionReservation = {
  ...
  lotCode: string | null;
  notes: string | null;
  /** Set when this reservation was auto-created from a specific bundle in Start Production (Section 3). */
  bundleId: string | null;
  /** The reserved bundle's display number (e.g. "Bundle 3"), joined in for display — null for manual reservations. */
...
type Db = {
  ...
  inventory_bundles: { bundle_number: number } | null;
  created_at: string;
};
...
function map(r: Db): ProductionReservation {
  return {
    ...
    bundleNumber: r.inventory_bundles?.bundle_number ?? null,
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ProductionReservation[]> => {
      const { data, error } = await supabase
        ...
        .select(
          "id, production_order_id, material_id, quantity, lot_code, notes, bundle_id, inventory_bundles(bundle_number), created_at",
        )
