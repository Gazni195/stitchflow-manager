...
export type StartProductionStep = {
  operationId: string;
  workstationId: string | null;
  assignedTo: string | null;
};

export function useStartProductionV2() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (v: {
      designId: string;
      ...
      const { data, error } = await supabase.rpc("start_production_v2", {
        _design_id: v.designId,
        _order_quantity: v.orderQuantity,
        _start_date: v.startDate,
        _supervisor: v.supervisor,
        _bundle_ids: v.bundleIds,
        _steps: v.steps.map((s) => ({
          operation_id: s.operationId,
          workstation_id: s.workstationId,
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["material-bundles"] });
    },
  });
}
