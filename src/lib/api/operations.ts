...
async function uploadOperationImage(operationId: string, kind: "icon" | "logo", file: File | null | undefined) {
  ...
  if (error) throw error;
  // Bucket is private (signed-in users only) — store the object path and sign on read.
  return path;
}

// Resolve a stored operation image path into a short-lived signed URL.
export async function signOperationImage(path: string | null): Promise<string | null> {
  if (!path) return null;
  if (/^https?:\/\//.test(path)) return path;
  const { data, error } = await supabase.storage.from(OPERATION_ICON_BUCKET).createSignedUrl(path, 60 * 60);
